    ...
    if DistanceBetweenPoints(udg_Point, udg_Point2) < 300.00 then
        ...
        if udg_DungeonColdLevel > 0 then
            ...
            if udg_DungeonColdLevel == 2 then
                call UnitRemoveBuffBJ(0x4230304C, ArctUnit)
            else
                if udg_DungeonColdLevel == 1 then
                    call UnitRemoveBuffBJ(0x4230304B, ArctUnit)
                else
                    if udg_DungeonColdLevel == 0 then
                        call UnitRemoveBuffBJ(0x42303047, ArctUnit)
                    endif
                endif
            endif
...
function InitTrig_Dungeon_Cold_Hearth takes nothing returns nothing
    ...
    call TriggerRegisterUnitEvent(gg_trg_Dungeon_Cold_Hearth, ArctUnit, EVENT_UNIT_ISSUED_TARGET_ORDER)
